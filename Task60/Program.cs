...
int[,,] CreateMatrixRndInt(int rows, int columns, int depth)
{
    ...
    for (int i = 0; i < matrix.GetLength(0); i++)
    {
        for (int j = 0; j < matrix.GetLength(1); j++)
        {
            ...
            {
                  matrix[i, j, k] = GetTwotDigitValue(usedValue);
            }
...
int GetTwotDigitValue(bool[] usedValues)
{
...
{
    arr3d = CreateMatrixRndInt(arr3d.GetLength(0), arr3d.GetLength(1), arr3d.GetLength(2));
